...
### Fixed(2026-08-12 · 端点探测与交互)

- **探测 403/1010 根因修复**:urllib 默认 User-Agent(`Python-urllib/3.x`)被部分站点的
  Cloudflare 防护拦截(如 OpenCodeGo → HTTP 403 error code 1010),请求已带浏览器 UA,
  密钥/模型/端点配置无需改动即可探测
- **输入框拖选修复**:在 Base URL / 模型 / 密钥输入框内拖选文本不再触发整卡拖拽
- **编辑状态保留**:探测/定时刷新/增删触发重渲染时,打开的编辑面板与输入内容不再被覆盖;
  定时刷新(10s)在用户编辑输入框时自动跳过
- 探测测试图改用 vision-test.png(共享目录已内置),替代 1×1 纯色图

### Added(2026-08-12 · 端点实测)

- **TEST tab**:上传图片/截图 → 选择目标节点 → 实测该端点是否支持多模态输入
  (真实图片验证,展示描述结果/错误与耗时)
...
- **色彩增强 `--colors`**(vision.py CLI + MCP `colors` 参数):对象颜色附加精确
  HEX 色值,供设计复刻/取色
- **自适应增强 `--auto`**(CLI + MCP `auto` 参数):视觉模型按图片内容自决是否
  附加坐标/色值(UI 截图自动开坐标、纯内容图保持简洁),显式档位优先
